...
Automatic replay of the queue patch stack onto `rust-v0.119.0` stopped at:
...
1. Pull this candidate branch:
   ...
   # or
   git fetch origin candidate/queue/rust-v0.119.0
   git switch -C candidate/queue/rust-v0.119.0 --track origin/candidate/queue/rust-v0.119.0
   ```

2. Fetch the upstream release tag:

   ```bash
   git fetch https://github.com/openai/codex refs/tags/rust-v0.119.0:refs/tags/rust-v0.119.0
   ```
...
6. Remove this helper note and push the updated branch:
   ...
   git push origin HEAD:candidate/queue/rust-v0.119.0
   ```
