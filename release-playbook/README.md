...
Maintain the candidate patch stack in this order:

1. stable internal fix commits that should keep carrying forward
2. one current-branch adaptation/consolidation commit for release-specific alignment
3. unstable or still-being-validated fix commits that should remain easy to amend, drop, or move

When a fix becomes stable, move it below the adaptation commit in the next cleanup pass. Keep
release-specific churn, conflict resolution, snapshot refreshes, and playbook maintenance in the
adaptation commit so the reusable fix stack stays clear.
