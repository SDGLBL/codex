...
async fn new_command_exec_app_server(codex_home: &Path) -> Result<TestAppServer> {
    new_command_exec_app_server_with_env(codex_home, &[]).await
}

async fn new_command_exec_app_server_with_env(
    codex_home: &Path,
    env_overrides: &[(&str, Option<&str>)],
) -> Result<TestAppServer> {
    let home_env = codex_home.to_string_lossy().to_string();
    let mut overrides = vec![
        ("HOME", Some(home_env.as_str())),
        ("FNM_ARCH", None),
        ("FNM_COREPACK_ENABLED", None),
        ("FNM_DIR", None),
        ("FNM_LOGLEVEL", None),
        ("FNM_MULTISHELL_PATH", None),
        ("FNM_NODE_DIST_MIRROR", None),
        ("FNM_RESOLVE_ENGINES", None),
        ("FNM_VERSION_FILE_STRATEGY", None),
        ("NVM_BIN", None),
        ("NVM_CD_FLAGS", None),
        ("NVM_DIR", None),
        ("NVM_INC", None),
    ...
    TestAppServer::new_with_env(codex_home, &overrides).await
}
...
async fn command_exec_without_streams_can_be_terminated() -> Result<()> {
    ...
    let mut mcp = new_command_exec_app_server(codex_home.path()).await?;
...
async fn command_exec_env_overrides_merge_with_server_environment_and_support_unset() -> Result<()>
{
    ...
    let codex_home = TempDir::new()?;
    create_config_toml(codex_home.path(), &server.uri(), "never")?;
    let mut mcp = new_command_exec_app_server_with_env(
        codex_home.path(),
