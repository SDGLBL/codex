...
fn retain_compacted_history_items(
    ...
    for group in history_item_groups(items) {
        if matches!(
            ...
            pending_reasoning = Some(group);
            continue;
        }
        if matches!(&group.source.item, ResponseItem::Reasoning { .. }) {
            pending_reasoning = None;
            continue;
        }
        if matches!(&group.source.item, ResponseItem::Message { role, .. } if role == "assistant") {
            if let Some(reasoning) = pending_reasoning.take() {
                retained.extend(reasoning.into_items());
                retained.extend(group.into_items());
            } else if let Some(summary) = assistant_message_as_user_summary(&group.source.item) {
                retained.push(ResponseItemEnvelope::new(summary));
            }
            continue;
        }

        pending_reasoning = None;
        if should_keep_non_reasoning_compacted_history_item(&group.source.item) {
            retained.extend(group.into_items());
        }
    }
