...
fn retain_compacted_history_items(items: Vec<ResponseItem>) -> Vec<ResponseItem> {
    ...
    for group in history_item_groups(items) {
        ...
        if matches!(&group.source, ResponseItem::Reasoning { .. }) {
            pending_reasoning = None;
            continue;
        }
        if matches!(&group.source, ResponseItem::Message { role, .. } if role == "assistant") {
            if let Some(reasoning) = pending_reasoning.take() {
                retained.extend(reasoning.into_items());
                retained.extend(group.into_items());
            } else if let Some(summary) = assistant_message_as_user_summary(&group.source) {
        ...
        pending_reasoning = None;
        if should_keep_non_reasoning_compacted_history_item(&group.source) {
            retained.extend(group.into_items());
        }
    }
